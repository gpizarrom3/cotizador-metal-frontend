...
import {
  guardarItemCatalogo, obtenerCatalogo, actualizarItemCatalogo, eliminarItemCatalogo,
  obtenerCatalogoGlobal, guardarItemCatalogoGlobal, actualizarItemCatalogoGlobal, eliminarItemCatalogoGlobal,
...
export default function Catalogo() {
  ...
  const filteredGlobal = globalItems.filter((i) =>
    i.nombre?.toLowerCase().includes(search.toLowerCase()) ||
    i.proveedor?.toLowerCase().includes(search.toLowerCase())
  )
  ...
  const abrirEditar = (i, global = false) => {
    setEditando(i.id)
    setIsGlobalEdit(global)
    setForm({
      nombre: i.nombre || '', proveedor: i.proveedor || '', formato: i.formato || '',
      precio_unitario: i.precio_unitario || '', unidad: i.unidad || '',
      tipo: i.tipo || '', peso_por_metro: i.peso_por_metro || '',
    })
    setShowModal(true)
  }
  const cerrarModal = () => { setShowModal(false); setEditando(null); setIsGlobalEdit(false); setForm(EMPTY_FORM) }
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      if (isGlobalEdit) {
        if (editando) {
          await actualizarItemCatalogoGlobal(editando, datos)
          setGlobalItems((prev) => prev.map((i) => i.id === editando ? { ...i, ...datos } : i))
        } else {
          const id = await guardarItemCatalogoGlobal(datos)
          setGlobalItems((prev) => [...prev, { id, ...datos }].sort((a, b) => a.nombre.localeCompare(b.nombre)))
        }
      } else {
        if (editando) {
  ...
  const handleEliminar = (id, global = false) => setConfirmDelete({ open: true, id, global })

  const ejecutarEliminar = async () => {
    const { id, global } = confirmDelete
    setConfirmDelete({ open: false, id: null, global: false })
    try {
      if (global) {
        await eliminarItemCatalogoGlobal(id)
    ...
    <DashboardLayout>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-1">{items.length} propios · {globalItems.length} globales · {CATALOGO_BASE.length} en catálogo base</p>
        </div>
        {activeTab === 'mis' && (
          <button onClick={abrirNuevo} className="btn-primary">+ Nuevo material</button>
        )}
        {activeTab === 'global' && isAdmin && (
          <button onClick={abrirNuevoGlobal} className="btn-primary">+ Agregar global</button>
        )}
      </div>
      ...
      <div className="flex gap-1 mb-5 bg-slate-900 p-1 rounded-xl w-fit">
        ...
        <button
          ...
          className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${activeTab === 'global' ? 'bg-slate-700 text-white' : 'text-slate-400 hover:text-slate-200'}`}
        >
          Catálogo global
          {globalItems.length > 0 && (
            <span className="ml-2 text-xs bg-teal-600/30 text-teal-400 px-1.5 py-0.5 rounded-full">{globalItems.length}</span>
          )}
        </button>
        <button
      ...
      <div className="mb-5">
        <div className="relative">
          ...
          <input
            ...
            placeholder={activeTab === 'base' ? 'Buscar en catálogo base...' : 'Buscar por nombre o proveedor...'}
            value={search}
      ...
      {activeTab === 'global' && (
        <div className="card">
          {!isAdmin && (
            <p className="text-xs text-slate-500 mb-4">Materiales disponibles para todos los usuarios Pro. Solo lectura.</p>
          )}
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="table-header">
                  ...
                  {isAdmin && <th className="text-center px-4 py-3 rounded-r-lg">Acciones</th>}
                  {!isAdmin && <th className="px-4 py-3 rounded-r-lg" />}
                </tr>
              </thead>
              <tbody>
                {filteredGlobal.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="text-center py-10 text-slate-500">
                      {globalItems.length === 0
                        ? isAdmin
                          ? 'Aún no hay materiales globales. Agrega uno con el botón de arriba.'
                          : 'Aún no hay materiales globales disponibles.'
                        : 'No se encontraron materiales.'}
                    </td>
                  </tr>
                ) : (
                  filteredGlobal.map((i) => (
                    <MaterialRow
                      key={i.id}
                      i={i}
                      onEdit={isAdmin ? () => abrirEditar(i, true) : null}
                      onDelete={isAdmin ? () => handleEliminar(i.id, true) : null}
                    />
                  ))
                )}
              </tbody>
          ...
          <div className="card w-full max-w-md">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-semibold text-white">
                {editando
                  ? isGlobalEdit ? 'Editar material global' : 'Editar material'
                  : isGlobalEdit ? 'Nuevo material global' : 'Nuevo material'}
              </h2>
            ...
            <div className="space-y-4">
              ...
              <p className="text-[11px] text-slate-600 -mt-1">
                Punto <strong className="text-slate-500">(·)</strong> para decimales, sin separador de miles — ej: <span className="text-slate-500">1500.50</span>
              </p>
      ...
      <ConfirmModal
        ...
        message={confirmDelete.global
          ? 'Este material será eliminado del catálogo global para todos los usuarios.'
