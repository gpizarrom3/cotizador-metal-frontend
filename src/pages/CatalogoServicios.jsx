...
import {
  ...
  obtenerCatalogoServiciosGlobal,
  guardarItemCatalogoServiciosGlobal,
  actualizarItemCatalogoServiciosGlobal,
  eliminarItemCatalogoServiciosGlobal,
...
function ServicioRow({ i, onEdit, onDelete }) {
  return (
...
export default function CatalogoServicios() {
  ...
  const filteredGlobal = globalItems.filter(
    (i) =>
      i.nombre?.toLowerCase().includes(search.toLowerCase()) ||
      i.descripcion?.toLowerCase().includes(search.toLowerCase())
  )
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      if (isGlobalEdit) {
        if (editando) {
          await actualizarItemCatalogoServiciosGlobal(editando, datos)
          setGlobalItems((prev) => prev.map((i) => i.id === editando ? { ...i, ...datos } : i))
        } else {
          const id = await guardarItemCatalogoServiciosGlobal(datos)
  ...
  const ejecutarEliminar = async () => {
    ...
    try {
      if (global) {
        await eliminarItemCatalogoServiciosGlobal(id)
    ...
    <DashboardLayout>
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-1">{items.length} propios · {globalItems.length} globales</p>
        </div>
        {activeTab === 'mis' && (
      ...
      {activeTab === 'global' && (
        <div className="card">
          {!isAdmin && (
            <p className="text-xs text-slate-500 mb-4">Servicios disponibles para todos los usuarios Pro. Solo lectura.</p>
          )}
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="table-header">
                  ...
                  <th className="text-left px-4 py-3">Unidad</th>
                  <th className="text-right px-4 py-3">Precio ref.</th>
                  <th className="text-center px-4 py-3 rounded-r-lg">{isAdmin ? 'Acciones' : ''}</th>
                </tr>
              </thead>
              <tbody>
                {filteredGlobal.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="text-center py-10 text-slate-500">
                      {globalItems.length === 0
                        ? isAdmin
                          ? 'Aún no hay servicios globales. Agrega uno con el botón de arriba.'
                          : 'Aún no hay servicios globales disponibles.'
                        : 'No se encontraron servicios.'}
                    </td>
                  </tr>
                ) : (
          ...
          <div className="card w-full max-w-md">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-semibold text-white">
                {editando
                  ? isGlobalEdit ? 'Editar servicio global' : 'Editar servicio'
                  : isGlobalEdit ? 'Nuevo servicio global' : 'Nuevo servicio'}
              </h2>
      ...
      <ConfirmModal
        ...
        title={confirmDelete.global ? 'Eliminar servicio global' : 'Eliminar servicio'}
        message={confirmDelete.global
          ? 'Este servicio será eliminado del catálogo global para todos los usuarios.'
          : 'Este servicio será eliminado de tu catálogo permanentemente.'}
